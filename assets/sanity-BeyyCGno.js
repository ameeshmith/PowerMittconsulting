...
As carbon capture facilities expand across Australia, early electrical engineering involvement during FEED ensures optimal motor drive selection, substation layout efficiency, and seamless grid compliance.
    `}],o="powermitt_custom_articles",c="powermitt_deleted_articles";function l(){try{const e=localStorage.getItem(o);return e?JSON.parse(e):[]}catch{return[]}}function g(){try{const e=localStorage.getItem(c);return e?JSON.parse(e):[]}catch{return[]}}function f(e){const t=l(),r=g().filter(s=>s!==e.slug);localStorage.setItem(c,JSON.stringify(r));const i=t.findIndex(s=>s.slug===e.slug||s.id===e.id);return i>=0?t[i]={...t[i],...e,publishedAt:e.publishedAt||new Date().toISOString().split("T")[0]}:t.unshift({...e,id:e.slug||`art-${Date.now()}`,publishedAt:e.publishedAt||new Date().toISOString().split("T")[0],author:e.author||{name:"Dinesh Mithanthaya",role:"Principal Power Engineer",avatar:"/assets/images/hero-nature-energy.jpg",bio:"Over 20 years of specialist experience in electrical power systems, grid connection studies, and heavy industrial infrastructure across Australia."}}),localStorage.setItem(o,JSON.stringify(t)),t}function v(e){const n=l().filter(i=>i.slug!==e&&i.id!==e);localStorage.setItem(o,JSON.stringify(n));const r=g();return r.includes(e)||(r.push(e),localStorage.setItem(c,JSON.stringify(r))),!0}async function m(){const e=l(),t=g(),n=new Map;return e.forEach(i=>n.set(i.slug,i)),h.forEach(i=>{!n.has(i.slug)&&!t.includes(i.slug)&&n.set(i.slug,i)}),Array.from(n.values()).filter(i=>!t.includes(i.slug))}async function w(e){return(await m()).find(n=>n.slug===e||n.id===e)||null}async function b(e,t=2){return(await m()).filter(r=>r.slug!==e&&r.id!==e).slice(0,t)}function S(){const e=l(),t=g();return{version:1,app:"PowerMitt Consulting",exportedAt:new Date().toISOString(),customArticles:e,deletedArticles:t}}function E(e,t="merge"){if(!e||typeof e!="object")throw new Error("Invalid backup file: content is not a valid JSON object.");const n=Array.isArray(e.customArticles)?e.customArticles:Array.isArray(e)?e:[],r=Array.isArray(e.deletedArticles)?e.deletedArticles:[];if(t==="replace")return localStorage.setItem(o,JSON.stringify(n)),localStorage.setItem(c,JSON.stringify(r)),{count:n.length};const i=l(),s=g(),d=new Map;i.forEach(a=>d.set(a.slug||a.id,a)),n.forEach(a=>d.set(a.slug||a.id,a));const u=Array.from(d.values()),p=Array.from(new Set([...s,...r]));return localStorage.setItem(o,JSON.stringify(u)),localStorage.setItem(c,JSON.stringify(p)),{count:u.length}}export{w as a,b,y as c,v as d,S as e,m as g,E as i,f as s};
